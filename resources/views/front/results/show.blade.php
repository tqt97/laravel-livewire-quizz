							...
							@forelse($users as $user)
								<tr @class(['bg-slate-100' => auth()->user() && auth()->user()->name == $user->name])>
									...
									<td>{{ $user->correct }} / {{ $total_questions }}
										(time: {{ intval($user->time_spent / 60) }}:{{ gmdate('s', $user->time_spent) }}
										minutes)</td>
								</tr>
							@empty
								<tr>
										...
										<td>
											@foreach($result->question->questionOptions as $option)
																		<li @class([
																			...
																			@if ($result->option_id == $option->id) <span class="italic">(your answer)</span>
																			@endif
																		</li>
											@endforeach
										...
										<tr>
											...
											<td>
												...
												@if ($result->question->more_info_link)
													<div class="mt-4">
														Read more:
														<a href="{{ $result->question->more_info_link }}" class="hover:underline"
															target="_blank">
